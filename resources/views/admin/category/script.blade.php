<script>
    ...
    function renderOptionsWithoutPricing(options = [], index) {
        ...
        html +=
            `</div><button type="button" class="btn btn-primary btn-sm mt-2 btn-add-option" data-index="${index}">Add Option</button>`;
        return html;
    }
    ...
    function renderDynamicOptionsHTML(field, index, pricing = {}, subCategory = null) {
      ...
      <div class="options-wrapper" id="options_wrapper_${index}_${subCategory.replace(/\s+/g, '_')}">`;
        ...
        setTimeout(() => {
            $(`#options_wrapper_${index}_${subCategory.replace(/\s+/g, '_')} .option-row input[type="text"]`)
                .each(function() {
                    updatePricingInputName($(this));
                });
        }, 0);
    ...
    $(document).on('click', '.btn-edit-category', function() {
        ...
        $.ajax({
            ...
            success: res => {
                ...
                category.adders.forEach(a => addAdderRow(a.name, a.price, a.type, a.min_qty, a
                    .max_qty));
                ...
                configFields.fields.forEach(field => {
                    if (field.type === 'dynamic_select') {
                        Object.keys(field.options).forEach(subCat => {
                            const parentInput = $(
                                `#configuration_fields_container .parent-option-input`
                            ).filter(function() {
                                return $(this).val() === subCat;
                            });
                            const parentRowId = parentInput.data('row-id');
                            $('#configuration_fields_container').append(
                                `<div id="dynamic_section_${index}_${parentRowId}" class="mt-3"><h6 class="mt-3">${subCat}</h6>${renderDynamicOptionsHTML(field, index, pricing, subCat)}</div>`
                            );
                        });
                        index++;
                    } else {
