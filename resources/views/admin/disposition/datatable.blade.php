<script>
    ...
    (function($) {
        ...
        $(function() {
            if ($.fn.DataTable.isDataTable('#disposition-table')) {
                $('#disposition-table').DataTable().destroy();
            }

            category_table = $('#disposition-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: ({
                    url: "{{ route('admin.disposition.table-data')}}",
                    method: "POST",
                    data: function(d) {
                        d._token = '{{ csrf_token() }}'
                    },
